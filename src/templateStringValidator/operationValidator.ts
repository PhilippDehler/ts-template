...
> = Type extends keyof TSchema
  ? Operation extends keyof TSchema[Type]
    ? "returnType" extends keyof TSchema[Type][Operation]
      ? TSchema[Type][Operation]["returnType"] extends StringKeys<TSchema>
        ? TSchema[Type][Operation]["returnType"] extends keyof TSchema
          ? TSchema[Type][Operation]["returnType"] & keyof TSchema
          : never
        : never
      : never
    : never
...
> = FromMaybe<
  Maybe<
    BrutForceOperationReturnType<TSchema, Type, Operation>,
    TSchema["typeDefinition"]["DEFAULT"]["key"]
  >
> &
  keyof TSchema;

export type ValidateOperationKey<
  Key extends string,
  TOperationLookUp extends OperationLookUp
> = If<
  NeedsSuggestions<Key>,
  Maybe<never, SuggestionMsg<StringKeys<TOperationLookUp>>>,
  Maybe<
    If<
      HasPartialMatch<Key, StringKeys<TOperationLookUp>>,
      SuggestionMsg<StringKeys<TOperationLookUp> & `${Key}${string}`>
    >,
...
> = OperationKeyAndArgs<Operation> extends {
  ...
  ? [
      ...
      ...If<
        FromMaybe<
          ValidateOperationKey<OperationKey, TOperationLookUp>
        > extends keyof TOperationLookUp
          ? true
          : false,
        If<
          IsEmptyArray<
            TOperationLookUp[FromMaybe<
              ValidateOperationKey<OperationKey, TOperationLookUp>
            >]["args"]
          >,
          [],
  ...
  : [
      Maybe<
        never,
        ErrorMsg<`Operation couldn't be parsed in ValidateOperation. Received:${Operation}`>
      >
    ];
...
> = Operation extends [
  ...
  ? TSchema[InitalType] extends OperationLookUp
    ...
    : [
        ...
        Maybe<
          never,
          ErrorMsg<`Inital type is not matching the schema keys in ValidateOperations. Received: ${InitalType &
            string}`>
        >
      ]
  : TAgg;
